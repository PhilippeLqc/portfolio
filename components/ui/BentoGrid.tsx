...
export const BentoGridItem = ({
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText(process.env.NEXT_PUBLIC_EMAIL_HOST!);
    setCopied(true);
  };
  return (
    <div
    ...
    >
      <div className={`${id === 6 && "flex justify-center"} h-full`}>
        ...
        >
          ...
          <div
            className={cn(
              "font-sans font-bold text-lg lg:text-xl max-w-96 z-10",
              titleClassName
            )}
          >
            {title}
          ...
          {id === 2 && <GridGlobe />}
          {id === 3 && (
            <div className="flex gap-1 lg:gap-1 w-fit absolute -right-3 lg:right-4">
              <div className="flex flex-col gap-3 lg:gap-2">
                {["MySQL", "Java", "Angular", "Spring"].map((tech) => (
                  <span
              ...
              <div className="flex flex-col gap-3 lg:gap-2">
                <span
                  className="lg:py-4 lg:px-4 py-2 px-3 text-xs lg:text-base opacity-50 
                    lg:opacity-100 rounded-lg text-center bg-[#10132E]"
                />
                {["React.js", "Next.js", "Tailwind", "MongoDB"].map((tech) => (
                  <span
                    className="lg:py-4 lg:px-4 py-2 px-3 text-xs lg:text-base opacity-50 
                    lg:opacity-100 rounded-lg text-center bg-[#10132E]"
                    key={tech}
